...
- Go 1.26 (constitution-mandated) + govips v2.18.0 (libvips bindings — `Orientation()`, `RemoveMetadata()`, format exports), mimetype v1.4.13, chi v5.2.5, pgx v5.9.1, sqlc, zap v1.27.1 (018-image-orient-dims)
- PostgreSQL (Alkemio shared DB; this service owns the `file` table and adds `content_metadata` JSONB) and local filesystem (file bytes; content-addressed by SHA3-256, unchanged) (018-image-orient-dims)
