...
type DocumentRepo interface {
	...
	//
	// It returns the AUTHORITATIVE post-update document — mapped from the row's
	// full RETURNING while it is UPDATE-locked in the same statement — so the
	// caller can build the ENTIRE response from one consistent snapshot without a
	// reload. That snapshot reflects both the applied metadata SETs and any
	// concurrent content-replace already committed (which swaps externalID/size +
	// dims WITHOUT bumping version), so response fields can never tear against
	// each other. The document is zero-valued when the update matched no row.
	//
	...
	UpdateMetadata(ctx context.Context, id uuid.UUID, meta model.DocumentMetadataUpdate, version int) (model.Document, error)
