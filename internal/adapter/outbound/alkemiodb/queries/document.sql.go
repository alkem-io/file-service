...
func (q *Queries) GetDocumentsByIDs(ctx context.Context, dollar_1 []pgtype.UUID) ([]GetDocumentsByIDsRow, error) {
	rows, err := q.db.Query(ctx, getDocumentsByIDs, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetDocumentsByIDsRow{}
	for rows.Next() {
