...
func run() int {
	...
	// libvips decode never runs on a read/PATCH/copy. Runs under bgCtx so a shutdown stops the sweep
	// promptly instead of scanning on through the drain; the next boot resumes where it left off.
	go runDimsBackfill(bgCtx, fileSvc, logger)
