...
> **Known generator issue**: `openapi.yaml` is generated by the workspace
> `apispec` tool, whose flow analysis misattributes the
> `RejectedContentResponse` schema to the 400 entry of this operation (and
> drops the 422 schema) when the handler's error switch writes different
