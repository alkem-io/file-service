...
> **Resolved generator issue** (2026-06-12): the apispec flow-analysis bug
> that misattributed the `RejectedContentResponse` schema to the 400 entry
> ([antst/go-apispec#30](https://github.com/antst/go-apispec/issues/30)) is
> fixed in apispec v0.4.15; the CI pin is bumped and `openapi.yaml` is
> regenerated — the 422 entry now correctly references
> `RejectedContentResponse`. Handler tests remain the enforcement.
