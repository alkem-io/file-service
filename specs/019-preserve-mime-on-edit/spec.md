...
- Q: How are new (blank) documents actually created — recheck against server and WOPI-service code? → A: Verified: the server stages a 0-byte buffer with the canonical office MIME and a filename carrying the proper extension; this service's create path trusts the declared MIME for empty content and validates it against the bucket allow-list, so fresh placeholders are stored correctly. One correction from the recheck: the replace path deletes the previous content blob once unreferenced, so recovery of prior content during remediation is only possible where the old blob is still referenced by another document.
- Q: Doesn't Collabora itself create a default new document when it opens a zero-length file? → A: Yes — confirmed by Collabora: a zero-byte file served with a proper extension is handled under the normal edit action and Collabora "create[s] a default blank document"; the `editnew` action is not needed (and Collabora does not advertise it in discovery). The blank-create flow as built is the Collabora-supported pattern and works in production. A legitimate first save therefore always carries real template bytes, so rejecting empty replacements (FR-003a) cannot break blank-document creation. The source of the four historical empty bodies remains undetermined (host/transport body loss is a known class of cause elsewhere); FR-008's rejection logging will identify it if it recurs.
